...
def chisq_test(
    ...
    if (namer in SIMULATION_MODELS) and ("DualPol" not in namer) and ("Split" not in namer):
        fit_kwargs_r["pol_sel"] = pol_sel
        eval_kwargs_r["pol_sel"] = pol_sel
        
    fit_kwargs_u = {key: val for key, val in fit_kwargs.items()}
    if (nameu in SIMULATION_MODELS) and ("DualPol" not in nameu) and ("Split" not in nameu):
    ...
    out = containers.ChisqTest(mock=np.arange(nmock, dtype=np.int),
    ...
    if modelr.nfit > 0:
        ...
        resdr = scipy.optimize.minimize(modelr.negative_log_likelihood,
                                        param0r, method=method, bounds=boundr,
                                        options=options)
    ...
    resdu = scipy.optimize.minimize(modelu.negative_log_likelihood,
                                    param0u, method=method, bounds=boundu,
                                    options=options)
    ...
    for mm in range(nmock):
        ...
        if modelr.nfit > 0:
            resr = scipy.optimize.minimize(modelr.negative_log_likelihood,
                                           param0r, method=method, bounds=boundr,
                                           options=options)
        ...
        resu = scipy.optimize.minimize(modelu.negative_log_likelihood,
                                       param0u, method=method, bounds=boundu,
                                       options=options)
